...
var firebaseConfig = {
  ...
  measurementId: "G-FHC9QYRKSW"
    ...
    var petwebDB = firebase.database().ref("Training");
    
    document.getElementById("Training").addEventListener("submit", submitForm);
    
    function submitForm(e) {
      e.preventDefault();
      var service = getElementVal("service");
      var pname = getElementVal("pet");
      ...
      saveMessages(name, service, pname, number, email, date, time, description);
    ...
    //   reset the form
    document.getElementById("Training").reset();
    }
    
    const saveMessages = (name,service, pname, number, email, date, time, description) => {
